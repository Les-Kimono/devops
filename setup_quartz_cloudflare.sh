...
for env_file in "$SCRIPT_DIR/.env" "$HOME/.devops-website.env" "$ROOT/.env" "$ROOT/.env.cloudflare"; do
  if [ -f "$env_file" ]; then
    set -a
...
cd "$SITE_DIR"
# Only create Quartz scaffold if it doesn't exist (no content/ directory or no package.json)
...
case "$project_status" in
  ...
  404) wrangler pages project create "$PROJ" --production-branch "${CURRENT_BRANCH:-main}" || echo "Warn: wrangler create failed" >&2;;
